...
- READONLY1 adds a read-only observed Perakam value pass before parity status publishing. It parses visible dashboard tile text for `Masa Hadir` and `Masa Keluar`, records only normalized times/page state/source/timestamp/reason, and exposes a compact sanitized `observedPerakam` summary in the existing heartbeat status text for the webapp. It must not navigate aggressively, click attendance controls, submit forms, or publish credentials, cookies, raw HTML, screenshots, opaque URLs, selectors, or form values.
- READONLY2 uses READONLY1 snapshots for post-action verification after the existing local/manual-confirm guarded click path only. It captures observed values before the click, does not repeat the click during verification, retries read-only observation with bounded delays, and updates the existing completion record with sanitized observed-value status/reason. `already-present` means the website value existed before the local click; `verified-success` means the value appeared afterward; stale/login/unreachable states remain `verification-unknown`.
...
- Use `Sync now` after opening or refreshing the local Perakam browser when a fresh observed website value is needed in the webapp. Missing observed values can mean login-required, stale-session, unreachable page, or no visible website time yet; they are not completion records and do not authorize an action.
- Treat READONLY2 verification as evidence for monitored operation, not approval for unattended execution. The next implementation track is AUTO1 shadow mode only, which should observe and simulate decisions without real auto-clicking.
