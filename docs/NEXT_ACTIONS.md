...
- Next track options: deploy/smoke PARITY4B/PARITY5/PARITY6/PARITY7 Edge Functions when physically ready and explicitly approved, start PARITY8 read-only webapp monitoring after approval, RC small real-world observation tasks, or PARITY11 startup/sleep-wake field validation. Do not implement configured-action command execution, webapp code, additional migrations, or captive portal reconnect without explicit approval.
...
- Configured-action command execution and webapp access remain deferred until auth/pairing/control-path authorization is expanded beyond status, skip, schedule/completion, and dry-run command sync.
