...
- Next track options: run PARITY10C deployed field validation with `paritySync.remoteActionEnabled=false` first, then `true` only for a supervised test, add action-specific skip UI after local scheduler support is upgraded, RC small real-world observation tasks, or PARITY11 startup/sleep-wake field validation. Do not implement additional migrations, unattended execution, or captive portal reconnect without explicit approval.
