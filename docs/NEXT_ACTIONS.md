...
- Next track options: run DEPLOY1 from `docs/DEPLOY1_SAFE_LOOP_CHECKLIST.md` first with `paritySync.remoteActionEnabled=false`, safe commands only, and no configured website click; then run PARITY10C field validation only after DEPLOY1 passes and the user explicitly approves guarded remote-action testing. Other options are action-specific skip UI after local scheduler support is upgraded, RC small real-world observation tasks, or PARITY11 startup/sleep-wake field validation. Do not implement additional migrations, unattended execution, or captive portal reconnect without explicit approval.
