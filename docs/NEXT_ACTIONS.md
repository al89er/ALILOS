...
- Next track options: run the PARITY9D safe-loop smoke test in `docs/PARITY_SAFE_LOOP_SMOKE.md` when physically ready and explicitly approved, start PARITY10 guarded configured-action command design/implementation after approval, add action-specific skip UI after local scheduler support is upgraded, RC small real-world observation tasks, or PARITY11 startup/sleep-wake field validation. Do not implement configured-action command execution, additional migrations, or captive portal reconnect without explicit approval.
