...
- Phase UI-2 Settings editor completed: safe app settings can be viewed/edited from the Settings tab through typed IPC, with Supabase heartbeat/parity URL fields and Telegram/key credentials visually masked where appropriate and blank secret fields preserving existing local values.
...
- PARITY5 adds disabled-by-default Supabase skip-date sync through `/functions/v1/alilos-skip-sync`. Remote skip rows affect scheduling only. DEPLOY1C adds local remote-managed skip tracking so webapp skip OFF can remove desktop skips that were created or uploaded through sync, while local-only/legacy skips are preserved unless explicitly removed in the desktop Schedule tab.
...
- PARITY9 adds safe webapp command controls for status refresh, dry-run/check, recalculate today, and cancel confirmation. These create pending commands through `/functions/v1/alilos-command-sync`; PARITY10B adds guarded configured-action requests behind the disabled-by-default desktop `remoteActionEnabled` gate.
- PARITY9B aligns the webapp to the existing three-tab workflow: Dashboard, Skip dates, and Log history. Log history shows sanitized event summaries only.
- PARITY9C makes the Skip dates calendar interactive for whole-day skip/unskip through `/functions/v1/alilos-skip-sync`. It affects scheduling only and does not create configured-site actions.
- PARITY9D documents the deployed safe-loop smoke-test runbook in `docs/PARITY_SAFE_LOOP_SMOKE.md`. It is docs-only and covers Edge Function deployment checks, existing-device setup, desktop parity sync flags, webapp config, status publishing, dashboard reads, skip upsert/delete, safe command processing, schedule/completion visibility, sanitized logs, and rollback.
- PARITY10B guarded remote configured-action execution is implemented but disabled by default. The Edge Function can create pending `perform-configured-action` commands with constrained fields, the webapp shows guarded request buttons with confirmation/safety copy, and the desktop rejects unless `paritySync.remoteActionEnabled` is true in local config.
...
- READONLY2 adds post-action read-only verification for existing local/manual-confirm action paths. It captures observed Perakam values before the approved local click, then performs a bounded read-only observed-value check afterward and records `verified-success`, `already-present`, `verification-unknown`, or `verification-failed` in existing completion state. It does not add unattended action, new webapp action buttons, or enable `remoteActionEnabled`.
...
- Settings can edit selected operational values only, including the explicit parity sync safe-loop flags. Generated schedules, completion records, automation audit events, raw logs, target mappings, configured-site credentials, cookies/session data, screenshots, raw HTML, and personal identifiers remain outside editable settings.
- Supabase heartbeat has a legacy Settings-tab project URL editor and disabled-by-default sender skeleton. The newer parity sync Settings section is separate and is the path to use for DEPLOY1 Supabase/webapp smoke testing. PARITY8 now adds a read-only web/PWA dashboard shell, but it depends on deployed Edge Functions and synced data for live status.
- Supabase is not required for local scheduled operation. The repository has the S2A heartbeat schema migration, S2B sender skeleton, S3A docs-only schedule/completion sync plan, S3B schema-only schedule/completion migration, PARITY2 schema-only skip/log/status/command support, PARITY3 disabled desktop sync skeleton, PARITY4 gated status publishing, PARITY4B server-side status proxy, PARITY5 disabled skip-date sync, PARITY6 disabled schedule/completion sync, PARITY7 disabled dry-run/non-clicking command sync, PARITY8 read-only web/PWA monitoring, and PARITY10B guarded configured-action command handling. Runtime sync and remote action remain disabled by default.
- The web/PWA companion has PARITY8 read-only monitoring, PARITY9 safe non-clicking command controls, PARITY9B three-tab workflow alignment, PARITY9C whole-day skip/unskip calendar controls, the PARITY9D safe-loop smoke runbook, and PARITY10B guarded configured-action request buttons. It falls back to static mock data if the read proxy/config/data is unavailable.
...
- PARITY5/DEPLOY1C skip sync is implemented but disabled by default. It requires the deployed `alilos-skip-sync` Edge Function, uses publishable/anon desktop credentials only, adds remote skip dates to local scheduling, and removes only skips known to be remote-managed or uploaded/synced when the remote row disappears.
...
- PARITY7 command sync is implemented but disabled by default. It requires the deployed `alilos-command-sync` Edge Function, uses publishable/anon desktop credentials only, handles dry-run/non-clicking requests, and PARITY10B guarded configured-action requests remain blocked unless `paritySync.remoteActionEnabled` is explicitly true.
...
- After READONLY2, completion rows can show post-action observed-value verification status and sanitized reason text. Missing/stale/unreachable Perakam state remains `verification-unknown`; a logged-in dashboard with missing expected value after retries is `verification-failed`.
...
- The Settings tab can edit worker enable/interval, automation execution mode/interval/dry-run preparation, scheduler windows/grace/reminders, Perakam dashboard URL, legacy Supabase heartbeat enable/project URL/interval, and the disabled-by-default parity sync flags needed for DEPLOY1. Parity sync publishable/anon key input is blank on load and only replaces the stored key when a new non-service-role-looking key is entered.
