...
- PARITY9 adds safe webapp command controls for status refresh, dry-run/check, recalculate today, and cancel confirmation. These create pending commands through `/functions/v1/alilos-command-sync`; remote configured-action execution remains rejected/deferred.
...
- PARITY10A prepares guarded remote configured-action preflight only. The Edge Function can record a validated `perform-configured-action` preflight as rejected/deferred, the webapp shows disabled readiness surfaces, and the desktop still rejects remote configured-action commands.
...
- The web/PWA companion has PARITY8 read-only monitoring, PARITY9 safe non-clicking command controls, PARITY9B three-tab workflow alignment, PARITY9C whole-day skip/unskip calendar controls, the PARITY9D safe-loop smoke runbook, and PARITY10A disabled configured-action readiness surfaces. It has no active configured-action button and falls back to static mock data if the read proxy/config/data is unavailable.
...
- PARITY9B web workflow alignment is implemented. PARITY9C whole-day skip/unskip calendar controls are implemented through the existing skip-sync Edge Function. PARITY9D safe-loop deployment/smoke guidance is documented. PARITY10A remote configured-action preflight is implemented as rejected/deferred scaffolding only. Action-specific skip UI remains deferred.
...
- Webapp configured-action command execution remains deferred until auth/pairing/control-path authorization is explicitly expanded beyond safe non-clicking commands.
