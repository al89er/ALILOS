...
- PARITY9B aligns the webapp to the existing three-tab workflow: Dashboard, Skip dates, and Log history. Skip dates are visual/read-only calendar cells for now; log history shows sanitized event summaries only.
...
- The web/PWA companion has PARITY8 read-only monitoring, PARITY9 safe non-clicking command controls, and PARITY9B three-tab workflow alignment. It has no configured-action button and falls back to static mock data if the read proxy/config/data is unavailable.
...
- PARITY9B web workflow alignment is implemented. The dashboard read proxy can return monthly skip rows and recent sanitized event logs, but skip/unskip calendar toggles remain deferred.
